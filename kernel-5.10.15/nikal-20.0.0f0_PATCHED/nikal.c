...
typedef struct timespec64             nLinux_timeSpec;
...
//@ckutlu
typedef struct proc_ops             nLinux_procOps;
...
//@ckutlu
static nLinux_procOps nNIKAL250_procOps =
{
   .proc_open = nNIKAL250_procOpen,
   .proc_read = seq_read,
...
{
   return (nNIKAL100_tU32)msleep_interruptible(timeToSleepInMilliseconds);
}
   ...
   nNIKAL100_tU32 *systemMicroSeconds)
{
   ...
   ktime_get_real_ts64(&systemTime);
...
static unsigned long calculateTimerExpire( nNIKAL100_tU32 timeout )
{
   ...
   return msecs_to_jiffies(timeout) + jiffies + 1;
}
...
nNIKAL100_cc int nNIKAL1500_helperACPIProbe(void *dev, nNIKAL100_tDriver *driver)
{
   ...
   device->deviceInfo.version = 1;
   strncpy(device->deviceInfo.acpi.deviceID.id, acpiDevice->pnp.bus_id, sizeof(device->deviceInfo.acpi.deviceID.id));

   snprintf(device->nameString,
            min(sizeof(device->nameString), sizeof(acpiDevice->pnp.bus_id)+sizeof(bus_prefix)),
            "%s%s", bus_prefix, acpiDevice->pnp.bus_id);
...
nNIKAL100_cc const char* nNIKAL1_getACPIDeviceID(const void *osDevObject)
{
   ...
   return acpi_device_hid(acpiDev);
#else 
   return NULL;
#endif 
}
...
nNIKAL100_cc nNIKAL1_tFirmwareBuffer *nNIKAL1_loadFirmwareBuffer(const void *device, const nNIKAL100_tText *filename)
{
   ...
   struct device *dev;
   nNIKAL1_tFirmwareBuffer *firmware;
   const nNIKAL200_tDevice *d = device;
   const nNIKAL100_tDeviceInfo *devInfo = &d->deviceInfo;

   firmware = (nNIKAL1_tFirmwareBuffer *)nNIKAL100_malloc(sizeof(nNIKAL1_tFirmwareBuffer));
   if (firmware == NULL)
      return NULL;

   switch(devInfo->busType)
   {
      case nNIKAL100_kBusTypePCI:
   ...
   if (request_firmware(&fw_entry, filename, dev) == 0)
   {
      ...
      firmware->data = fw_entry->data;
      firmware->osFirmwareData = fw_entry;
   }
   else
   {
      nNIKAL100_free(firmware);
      firmware = NULL;
   }
...
{
   KAL_ASSERT(firmware != NULL);

   release_firmware(firmware->osFirmwareData);
   nNIKAL100_free(firmware);
}
...
nNIKAL100_mExportSymbol(nNIKAL1_sleepTimeoutInterruptible);
...
nNIKAL100_mExportSymbol(nNIKAL1_getACPIDeviceID);
...
nNIKAL100_mExportSymbol(nNIKAL1_loadFirmwareBuffer);
nNIKAL100_mExportSymbol(nNIKAL1_freeFirmwareBuffer);
